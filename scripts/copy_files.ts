...
let task_files = tasks.map(function (task_name) {
  ...
  // copy the node modules to the built task
  let nm_source = path.join(__dirname, "..", "tasks", task_name, "node_modules");
  let nm_target = path.join(output, "tasks", task_name)
  common.copyFolderRecursiveSync(nm_source, nm_target)
});
